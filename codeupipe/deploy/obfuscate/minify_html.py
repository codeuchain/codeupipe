"""MinifyHtml — minify HTML+CSS via html-minifier-terser."""
...
class MinifyHtml:
    ...
    Shells out to ``npx html-minifier-terser`` (or a local install).
    Falls back to whitespace-only compression if the tool is not available
    and ``strict=False`` (the default).
    ...
    def call(self, payload: Payload) -> Payload:
        html_list = payload.get("reassembled_html") or []
